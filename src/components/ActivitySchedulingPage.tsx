    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
        ...
        {onTriggerSyncAll && (
          <button
            ...
            className="rounded bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-xs px-3.5 py-2 cursor-pointer shadow-xs transition-colors"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="space-y-5">
          ...
          <div className="rounded border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 shadow-xs space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-100 border-b border-slate-100 pb-2 dark:border-slate-800 flex items-center gap-1.5">
            ...
            <div className="space-y-4 text-xs font-sans">
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-850 dark:text-slate-200 block text-xs">Sync on Startup</span>
                ...
                <button onClick={() => setSyncOnStartup(!syncOnStartup)} className="cursor-pointer">
                  {syncOnStartup ? <ToggleRight className="h-6 w-6 text-indigo-600" /> : <ToggleLeft className="h-6 w-6 text-slate-404" />}
                </button>
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-850 dark:text-slate-200 block text-xs">Sync on Internet Connect</span>
                ...
                <button onClick={() => setSyncOnInternet(!syncOnInternet)} className="cursor-pointer">
                  {syncOnInternet ? <ToggleRight className="h-6 w-6 text-indigo-600" /> : <ToggleLeft className="h-6 w-6 text-slate-404" />}
                </button>
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <span className="font-semibold text-slate-850 dark:text-slate-200 block text-xs">Automatic Retry</span>
                ...
                <button onClick={() => setAutoRetry(!autoRetry)} className="cursor-pointer">
                  {autoRetry ? <ToggleRight className="h-6 w-6 text-indigo-600" /> : <ToggleLeft className="h-6 w-6 text-slate-404" />}
                </button>
              </div>

              {autoRetry && (
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
                  <label className="text-[10px] text-slate-450 font-semibold uppercase block">Retry Interval</label>
                  <select
                    value={retryInterval}
                    onChange={(e) => setRetryInterval(e.target.value)}
                    className="w-full text-xs p-1.5 rounded border border-slate-200 bg-slate-50 dark:border-slate-805 dark:bg-slate-950 text-slate-705 outline-none font-medium"
                  >
          ...
          <div className="rounded border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 shadow-xs space-y-4">
            <div className="flex justify-between items-center border-b border-slate-100 pb-2 dark:border-slate-800">
              ...
              <button
                ...
                className="text-[10px] bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 font-bold px-2 py-0.5 rounded cursor-pointer transition-all uppercase"
              >
                + Create
              </button>
            </div>
            ...
            <div className="space-y-3">
              {syncGroups.map((g) => {
                ...
                return (
                  <div key={g.id} className="p-3 border rounded border-slate-100 dark:border-slate-800/80 bg-slate-50/40 dark:bg-slate-950/20 text-xs">
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-slate-800 dark:text-slate-200">{g.name}</span>
                      <span className={`px-1 rounded text-[9px] font-bold ${isGroupActive ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400' : 'bg-slate-100 text-slate-500'}`}>
                        {g.status}
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-500 space-y-0.5 mt-1">
                      <div>Active Connections: {g.requestCount} mapped</div>
                      <div>Period: {g.schedule}</div>
                    </div>
                    <div className="flex justify-end gap-1.5 mt-2 pt-1 border-t border-slate-100 dark:border-slate-800/50">
                      <button
        ...
        <div className="xl:col-span-2 space-y-5">
          ...
          <div className="rounded border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 shadow-xs space-y-4">
            ...
            <div className="overflow-x-auto border border-slate-150 dark:border-slate-800 rounded">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-950/80 font-semibold border-b border-slate-200 dark:border-slate-800 text-slate-450">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {schedules.map((s) => {
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/10 text-[11px]">
                        <td className="p-2.5 font-semibold text-slate-800 dark:text-slate-200">{s.requestName}</td>
                        <td className="p-2.5 text-slate-500">{s.scheduleType}</td>
                        <td className="p-2.5 font-mono text-slate-500">{s.nextRun}</td>
                        <td className="p-2.5">
                          <span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-bold ${isSchedActive ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-400' : 'bg-slate-100 text-slate-550'}`}>
                            {s.status}
                          </span>
                        </td>
                        <td className="p-2.5 text-right space-x-2">
                          ...
                          <button
                            ...
                            className="text-red-650 hover:underline text-[10px] cursor-pointer"
                          >
                            Delete
                          </button>
                        </td>
          ...
          <div className="rounded border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 border-b border-slate-100 pb-2 dark:border-slate-800">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-100 flex items-center gap-1.5">
                <Clock className="h-4 w-4 text-indigo-500" />
                Recent Activity
              </h3>

              <div className="flex gap-1 bg-slate-50 dark:bg-slate-950 border dark:border-slate-800 p-1 rounded">
                {(['all', 'success', 'failed', 'uploads', 'downloads'] as const).map((lvl) => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase cursor-pointer transition-all ${
                      filterType === lvl ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-205'
                    }`}
                  >
                    {lvl}
                  </button>
            ...
            <div className="space-y-3 font-mono text-xs max-h-[190px] overflow-y-auto">
              {recentEvents
                ...
                .map((ev) => (
                  <div key={ev.id} className="flex justify-between hover:bg-slate-50/50 dark:hover:bg-slate-900/30 p-1 rounded-sm border-b border-dashed dark:border-slate-850/40 text-[11px]">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-1.5">
                        <span className="text-slate-400">[{ev.time}]</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{ev.request}</span>
                        <span className={`px-1 py-0.1 text-[9px] font-bold rounded ${ev.status === 'SUCCESS' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20' : 'bg-red-50 text-red-700'}`}>
                          {ev.status}
                        </span>
                      </div>
                      <p className="text-slate-505 dark:text-slate-400 lowercase italic">{ev.msg}</p>
                    </div>
                    <span className="text-indigo-650 dark:text-indigo-400 font-bold tracking-wider text-[10px] bg-indigo-50 dark:bg-indigo-950/20 px-1 py-0.5 rounded self-start shrink-0 uppercase">
                      {ev.operation}
                    </span>
                  </div>
                ))}
      ...
      {createGroupModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-950/40 backdrop-blur-xs" onClick={() => setCreateGroupModalOpen(false)} />
          <form onSubmit={handleCreateGroup} className="relative w-full max-w-sm rounded border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 text-slate-800 dark:text-slate-200 z-50 shadow-2xl space-y-4">
            <div className="flex justify-between items-start border-b pb-2 dark:border-slate-800">
              <span className="font-bold text-xs uppercase text-slate-800 dark:text-slate-100">Create Sync Group</span>
              <button type="button" onClick={() => setCreateGroupModalOpen(false)} className="text-slate-400 hover:text-slate-650"><X className="h-4 w-4" /></button>
            </div>

            <div className="space-y-3 text-xs">
              <div className="space-y-1">
                <label className="text-slate-400 block font-semibold">Group Name</label>
                <input
                  type="text"
                  placeholder="e.g. Daily Reports Backup"
                  value={newGroupName}
                  onChange={(e) => setNewGroupName(e.target.value)}
                  className="w-full p-2 bg-slate-50 dark:bg-slate-950 border dark:border-slate-805 rounded outline-none"
                  required
                />
              </div>

              <div className="space-y-1">
                <label className="text-slate-400 block font-semibold">Schedule Period</label>
                <select
                  value={newGroupSchedule}
                  onChange={(e) => setNewGroupSchedule(e.target.value)}
                  className="w-full p-2 bg-slate-50 dark:bg-slate-955 border dark:border-slate-805 rounded outline-none"
                >
              ...
              <div className="space-y-1">
                <label className="text-slate-400 block font-semibold">Select Mapped Connections</label>
                <div className="space-y-1 max-h-[100px] overflow-y-auto border dark:border-slate-805 p-2 rounded bg-slate-50 dark:bg-slate-955">
                  {syncRequests.map((req) => (
                    <label key={req.id} className="flex items-center gap-2 cursor-pointer font-sans text-xs">
                      <input
                        type="checkbox"
            ...
            <div className="flex justify-end gap-2 pt-2 border-t dark:border-slate-800">
              <button
                ...
                className="px-3 py-1.5 border border-slate-200 dark:border-slate-800 bg-white text-slate-705 dark:bg-slate-800 dark:text-slate-300 text-xs font-semibold rounded cursor-pointer"
              >
                Cancel
              </button>
              <button
